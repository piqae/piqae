...
use piqae_agent_core::{AgentError, ContentStore, StoredContent};
use piqae_domain::UriAuthentication;
use std::{
...
fn ipv6_allowed(address: Ipv6Addr, allow_private: bool) -> bool {
    ...
    if let Some(compatible) = address.to_ipv4() {
        return ipv4_allowed(compatible, allow_private);
    }
    ...
    if segments[0] == 0x2002 {
        ...
        return ipv4_allowed(Ipv4Addr::new(a, b, c, d), allow_private);
    }
...
mod tests {
    ...
    async fn private_destinations_are_denied_by_default() {
        ...
        assert!(!ipv6_allowed(
            "::10.0.0.1".parse().expect("compatible address"),
            false
        ));
        assert!(!ipv6_allowed(
            "2002:0a00:0001::".parse().expect("6to4 private address"),
            false
        ));
        assert!(!ipv6_allowed(
            "2001:0000:0000:0000:0000:0000:f5ff:fffe"
                .parse()
                .expect("Teredo private client"),
            false
        ));
        assert!(ipv6_allowed(
            "::8.8.8.8".parse().expect("compatible public address"),
            false
        ));
    }
