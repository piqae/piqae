...
impl Executor for SupervisedExecutor {
    async fn submit(
        ...
        let job_id = executor_job_id(&submission.job_id)?;
...
/// Preserve the durable pre-v0.1.22 local idempotency identity while giving
/// the typed executor protocol a valid `JobId`. The synthetic namespace is
/// accepted only for the exact 128-bit lowercase-hex form generated by the
/// installed node; arbitrary local strings remain rejected.
fn executor_job_id(value: &str) -> Result<JobId, ExecutorFailure> {
    if let Ok(job_id) = value.parse::<JobId>() {
        return Ok(job_id);
    }
    let Some(encoded) = value.strip_prefix("job_local_") else {
        return Err(invalid_local_job_id());
    };
    if encoded.len() != 32
        || !encoded
            .bytes()
    ...
    let value = u128::from_str_radix(encoded, 16).map_err(|_| invalid_local_job_id())?;
    Ok(JobId::from_ulid(ulid::Ulid::from(value)))
}
...
mod tests {
    use super::{
        ...
        executor_job_id, stderr_evidence,
    };
    use std::sync::{Arc, Mutex};
    use tokio::io::AsyncWriteExt as _;

    #[test]
    fn translates_only_the_stable_local_idempotency_namespace() {
        let legacy = "job_local_1b50fe719e9491e92621324d46b437b9";
        assert!(executor_job_id(legacy).is_ok());
        assert!(executor_job_id(&piqae_domain::JobId::new().to_string()).is_ok());
        assert!(executor_job_id("job_emb_1b50fe719e9491e92621324d46b437b9").is_err());
        assert!(executor_job_id("job_local_1B50FE719E9491E92621324D46B437B9").is_err());
        assert!(executor_job_id("job_local_short").is_err());
    }
