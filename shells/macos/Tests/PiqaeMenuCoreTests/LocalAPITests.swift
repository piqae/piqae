...
final class LocalAPITests: XCTestCase {
    ...
    func testConfigurationUsesExplicitEnvironment() throws {
        let configuration = try LocalAPIConfiguration(
            environment: [
                "PIQAE_LOCAL_API_URL": "http://localhost:49100",
                "PIQAE_DATA_DIR": "/var/lib/piqae",
            ],
            homeDirectory: URL(fileURLWithPath: "/Users/test")
        )

        XCTAssertEqual(configuration.baseURL.absoluteString, "http://localhost:49100")
        XCTAssertEqual(configuration.tokenFile.path, "/var/lib/piqae/local.token")
    }

    func testConfigurationPreservesTheShippedStateDirectoryByDefault() throws {
        let configuration = try LocalAPIConfiguration(
            environment: [:],
            homeDirectory: URL(fileURLWithPath: "/Users/test")
        )
        ...
        XCTAssertEqual(
            configuration.tokenFile.path,
            "/Users/test/Library/Application Support/Spool/local.token"
        )
    }
    ...
    func testConfigurationPrefersExplicitTokenFile() throws {
        let configuration = try LocalAPIConfiguration(
            environment: [
                "PIQAE_LOCAL_TOKEN_FILE": "/tmp/piqae-shell-token",
                "PIQAE_DATA_DIR": "/var/lib/piqae",
            ]
        )

        XCTAssertEqual(configuration.tokenFile.path, "/tmp/piqae-shell-token")
    }
    ...
    func testProfileCaptureSessionRoutesAndCaptureToken() async throws {
        ...
        StubURLProtocol.handler = { request in
            ...
            XCTAssertEqual(
                request.value(forHTTPHeaderField: "X-Piqae-Capture-Token"),
                "capture-secret"
            )
