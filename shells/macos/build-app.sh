...
if [[ -n "$swift_archs" ]]; then
  ...
  for architecture in "${requested_archs[@]}"; do
    case "$architecture" in
      ...
      *) echo "PIQAE_SWIFT_ARCHS supports only arm64,x86_64" >&2; exit 2 ;;
    esac
  done
fi
...
case "$bundle" in
  ...
  *.app) ;;
  *) echo "PIQAE_APP_BUNDLE must end in .app" >&2; exit 2 ;;
esac
if [[ ! "$version" =~ ^[0-9A-Za-z][0-9A-Za-z.-]*$ ]]; then
  echo "PIQAE_VERSION is not a safe bundle version" >&2
  exit 2
fi
if [[ ! "$build_number" =~ ^[1-9][0-9]*$ ]]; then
  echo "PIQAE_BUILD_NUMBER must be a positive integer" >&2
  exit 2
fi
if [[ -n "$feed_url" || -n "$public_key" || -n "$signing_identity" ]]; then
  ...
  if [[ ! "$feed_url" =~ ^https://[A-Za-z0-9][A-Za-z0-9.-]*(:[0-9]+)?(/[A-Za-z0-9._~:/?%+\&=-]*)?$ ]]; then
    echo "PIQAE_SPARKLE_FEED_URL must be a safe HTTPS URL" >&2
    exit 2
  fi
  if [[ ! "$public_key" =~ ^[A-Za-z0-9+/=]+$ ]]; then
    echo "PIQAE_SPARKLE_PUBLIC_ED_KEY is not valid base64 text" >&2
    exit 2
  fi
  decoded_key_length=$(printf '%s' "$public_key" | base64 -D | wc -c | tr -d ' ')
  if [[ "$decoded_key_length" -ne 32 ]]; then
    echo "PIQAE_SPARKLE_PUBLIC_ED_KEY must decode to a 32-byte Ed25519 public key" >&2
    exit 2
  fi
fi
...
install -m 0755 "$binary_directory/PiqaeMenu" "$bundle/Contents/MacOS/PiqaeMenu"
install -m 0755 \
  "$binary_directory/PiqaePrintCoreReplay" \
  "$bundle/Contents/MacOS/PiqaePrintCoreReplay"
menu_rpaths=$(otool -l "$bundle/Contents/MacOS/PiqaeMenu")
if ! grep -F "@executable_path/../Frameworks" <<<"$menu_rpaths" >/dev/null; then
  install_name_tool \
    -add_rpath "@executable_path/../Frameworks" \
    "$bundle/Contents/MacOS/PiqaeMenu"
fi
...
if [[ -n "$feed_url" ]]; then
  ...
  "$plist_buddy" -c "Set :PiqaeUpdatesEnabled true" "$bundle/Contents/Info.plist"
...
if [[ -n "$signing_identity" ]]; then
  PIQAE_CODE_SIGN_IDENTITY="$signing_identity" \
