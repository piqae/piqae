...
public struct LocalAPIConfiguration: Equatable, Sendable {
    ...
    public init(baseURL: URL, tokenFile: URL) throws {
        guard Self.isSafeLoopbackURL(baseURL) else {
            throw LocalAPIError.invalidConfiguration(
                "PIQAE_LOCAL_API_URL must be an HTTP loopback URL"
            )
        }
    ...
    public init(
        ...
        let baseURL: URL
        if let rawURL = environment["PIQAE_LOCAL_API_URL"], !rawURL.isEmpty {
            guard let parsed = URL(string: rawURL) else {
                throw LocalAPIError.invalidConfiguration(
                    "PIQAE_LOCAL_API_URL is not a valid URL"
                )
            }
            baseURL = parsed
        } else {
        ...
        let tokenFile: URL
        if let path = environment["PIQAE_LOCAL_TOKEN_FILE"], !path.isEmpty {
            tokenFile = URL(fileURLWithPath: path)
        } else if let dataDirectory = environment["PIQAE_DATA_DIR"], !dataDirectory.isEmpty {
            tokenFile = URL(fileURLWithPath: dataDirectory)
                .appendingPathComponent("local.token", isDirectory: false)
        } else {
            tokenFile = homeDirectory
                // Keep the shipped state directory stable across the visible
                // Spool -> Piqae rename so existing nodes retain their token,
                // identity, profiles, and durable queue.
...
public final class LocalAPIClient: @unchecked Sendable {
    ...
    public func completeProfileCapture(
        ...
        try await request(
            ...
            additionalHeaders: ["X-Piqae-Capture-Token": session.captureToken]
        )
    }
