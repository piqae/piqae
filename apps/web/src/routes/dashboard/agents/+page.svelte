...
{#snippet actions()}
  ...
  <button class="button primary" onclick={openEnrolment}><Icon name="plus" size={13} /> Enrol agent</button>
{/snippet}
...
<dialog bind:this={enrolmentDialog} aria-labelledby="enrolment-title" onclose={resetEnrolmentSession}>
  ...
  >
    <header class="dialog-header">
      <div>
        <h2 id="enrolment-title">Enrol an agent</h2>
        <p>Create a short-lived, one-time credential for a new Spool service.</p>
      </div>
      <button
        class="icon-button"
        type="button"
    ...
    <div class="dialog-body">
      ...
      {#if enrolmentResultVisible && !enrolmentPending && form?.mutation === 'createEnrolment' && form?.enrolment}
        <section class="secret-result" aria-live="polite">
          <div>
            <strong>Enrolment token · shown once</strong>
    ...
    <footer class="dialog-footer">
      ...
      >{enrolmentPending ? 'Creating…' : 'Create token'}</button>
    </footer>
  </form>
</dialog>
