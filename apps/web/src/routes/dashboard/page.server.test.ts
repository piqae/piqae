...
function validForm(content = "%PDF-1.7\nfixture") {
  ...
  const document = new File([bytes], "packing-slip.pdf", {
      ...
      data: [
        {
          id: "prt_01",
          state: "online",
          profiles: [{ profile_id: "prof_a4", status: "stale" }],
        },
      ],
    });
...
describe("uncertain delivery resolution", () => {
  it("passes the exact idempotency request through and reports pending node acknowledgement", async () => {
    resolveUncertain.mockResolvedValue({
...
describe("scoped node operations", () => {
  ...
  it("requests an advisory refresh through the resolved managed tenant", async () => {
    ...
    expect(result).toMatchObject({
      ...
      nodeRefreshHint: { status: "pending" },
    });
    expect(requestNodeRefresh).toHaveBeenCalledWith(
      "agt_01",
      expect.stringMatching(/^dashboard-refresh-/),
    );
  });
  ...
  it("requires the scoped node name before removing a managed projection", async () => {
    ...
    const managedApi = {
      agents: vi.fn(async () => ({
        data: [{ id: "agt_01", name: "Kitchen iPad" }],
        nextCursor: null,
      })),
      removeNode,
    };
    ...
    await expect(removeNodeAction(event(wrong))).resolves.toMatchObject({
      status: 400,
    });
    ...
    confirmed.set(
      "expected_node_name",
      "Caller supplied name is not authoritative",
    );
  ...
  it("does not remove a node absent from the server-resolved managed workspace", async () => {
    ...
    await expect(removeNodeAction(event(form))).resolves.toMatchObject({
      status: 404,
    });
    expect(removeNode).not.toHaveBeenCalled();
  });
...
describe("dashboard state addressing", () => {
  it("keeps node operations visible when only runtime telemetry is unavailable", async () => {
    dashboardSource.mockReturnValue({
      api: {
        ...
        printers: async () => emptyPage,
        agents: async () => ({
          data: [{ id: "agt_01", name: "Warehouse Mac" }],
          nextCursor: null,
        }),
        destinations: async () => emptyPage,
        routes: async () => emptyPage,
  ...
  it("carries an uncertain-delivery filter from the URL into the view model", async () => {
    ...
    expect(data).toMatchObject({
      view: "jobs",
      stateFilter: "delivery_uncertain",
    });
  });

  it("widens a state that does not apply to the requested view", async () => {
    expect(
      await load(loadEvent("?view=printers&state=delivery_uncertain")),
    ).toMatchObject({
      view: "printers",
      stateFilter: "all",
    });
...
describe("managed customer selection", () => {
      ...
      data: [
        {
          ...
          agents: [
            {
              ...{ id: "agt_child", name: "Shop Mac", state: "online" },
              customer: {
                id: "wsp_child",
                externalId: "c4beta",
                name: "C4 Beta",
  ...
  it("fails closed before issuing child resource requests for an unowned customer", async () => {
    ...
    const data = await load({
      url: new URL(
        "https://piqae.test/dashboard?view=nodes&managed_customer=foreign",
      ),
      parent: async () => ({ meta: { platform: { accounts: true } } }),
    } as never);
    ...
    expect(data).toMatchObject({
      ...
      agents: [],
      dataError: {
        message: expect.stringMatching(/unavailable or is not owned/i),
      },
    });
  });
});
