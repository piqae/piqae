...
describe("PiqaeClient", () => {
  ...
  it("uses a stable retryable fallback for non-JSON resource upload failures", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
  ...
  it("retrieves a consolidated design specification", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
        new Response(JSON.stringify({ specification_revision: "spec_1" }), {
          status: 200,
        }),
      );
    const client = new PiqaeClient({ fetch: fetcher });
  ...
  it("confirms or clears a printer loaded-media observation", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
        new Response(JSON.stringify({ source: "main-roll", stock: null }), {
          status: 200,
        }),
      );
    const client = new PiqaeClient({ fetch: fetcher });
  ...
  it("verifies signed webhooks and rejects stale or changed bodies", async () => {
    ...
    const headers = {
      "piqae-timestamp": String(timestamp),
      "piqae-signature": `v1=${signature}`,
    };
    await expect(
      verifyWebhookSignature("whsec_test", body, headers, {
        now: timestamp * 1000,
      }),
    ).resolves.toBe(true);
    await expect(
  ...
  it("returns complete synced printer capability and profile snapshots", async () => {
    ...
    expect(page.data[0]?.native_options.InputSlot?.selected_choice).toBe(
      "Tray2",
    );
  ...
  it("creates and revokes environment-scoped API keys", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ id: "key_1", secret: "piq_test_x" })),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ id: "key_1", revoked_at: "now" })),
      );
    const client = new PiqaeClient({
    ...
    expect(String(revokeUrl)).toBe(
      "https://print.example.test/v1/api-keys/key_1",
    );
  ...
  it("keeps local-owner bootstrap tokens in headers and credentials in JSON bodies", async () => {
    ...
    expect(
      new Headers(bootstrapInit?.headers).get("x-piqae-bootstrap-token"),
  ...
  it("uploads declared content through the authenticated proxy without base64", async () => {
    ...
    expect(String(uploadUrl)).toBe(
      "https://print.example.test/v1/uploads/upl_01/content",
    );
  ...
  it("requires and forwards idempotency for encrypted job registration", async () => {
    ...
    const fetcher = vi
      ...
      .mockImplementation(async (url, init) => {
        if (String(url).endsWith("/v1/uploads") && init?.method === "POST")
          return Response.json(
            {
              ...upload,
              upload_url: "/v1/uploads/upl_cipher/content",
              upload_method: "PUT",
              upload_headers: {},
              requires_completion: false,
            },
        ...
        return Response.json(
          { id: "job_cipher", printer_id: "prt_1", state: "registered" },
          { status: 201 },
        );
      });
    const client = new PiqaeClient({
    ...
    const envelope: EncryptedJobEnvelope = {
      version: "piqae-encrypted-job-v3",
      suite: "ECDH-ES-P256+HKDF-SHA256+A256GCMKW+A256GCM",
      binding: {
        envelope_id: "env_012345678901234567890123",
        workspace_id: "wsp_1",
        environment_id: "env_1",
        content_type: "pdf",
        printer_id: "prt_1",
        target_id: "tgt_1",
        profile_revision: "prf_1:1",
        options: {
          bin: null,
          collate: null,
          color: null,
      ...
      recipients: [
        {
          ...
          ephemeral_public_key: `B${"A".repeat(86)}`,
    ...
    await client.jobs.createEncrypted(
      { target_id: "tgt_1", title: "Private", content_type: "pdf" },
      envelope,
      "encrypted-retry-1",
    );
    const jobCall = fetcher.mock.calls.find(([url]) =>
      String(url).endsWith("/v1/jobs"),
    );
    expect(new Headers(jobCall?.[1]?.headers).get("idempotency-key")).toBe(
      "encrypted-retry-1",
    );
    await expect(
      client.jobs.createEncrypted(
        { target_id: "tgt_1", title: "Private", content_type: "pdf" },
        envelope,
        "short",
      ),
      ...
      client.jobs.createEncrypted(
        {
          ...
          title: "Unsafe",
          content_type: "pdf",
          resolved_ticket_digest: "b".repeat(64),
        } as never,
        envelope,
        "encrypted-unsafe-1",
      ),
    ).rejects.toThrow(/cannot attach unbound/);
    await expect(
      client.jobs.createEncryptedResolved(
        { target_id: "tgt_1", title: "Private", content_type: "pdf" },
        envelope,
        {
          digest: "c".repeat(64),
          printer_id: "prt_other",
