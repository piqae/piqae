...
# Retain the shipped V1 state path so upgrades preserve identity and queues.
...
$PiqaeAgent = Join-Path $InstallDirectory "piqae-agent.exe"
...
if ($Mode -ne "local") {
    if (-not $ControlPlaneUrl) {
        $ControlPlaneUrl = [uri](Read-Host "Control-plane URL (for example https://piqae.example.com)")
    }
    ...
    try {
        $env:PIQAE_ENROLMENT_TOKEN = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($tokenPointer)
        $arguments = @(
            "--mode", $Mode,
        ...
        $enrolmentJson = (& $PiqaeAgent @arguments | Out-String)
        if ($LASTEXITCODE -ne 0) {
            throw "The control plane rejected enrolment."
        }
    } finally {
        $env:PIQAE_ENROLMENT_TOKEN = $null
...
$config = [ordered]@{
    ...
    executor_path = (Join-Path $InstallDirectory "piqae-executor-windows.exe")
    profile_host_path = (Join-Path $InstallDirectory "piqae-profile-host-windows.exe")
...
if (-not $DoNotStart) {
    & (Join-Path $InstallDirectory "Stop-Piqae.ps1")
    & (Join-Path $InstallDirectory "Start-Piqae.ps1")
