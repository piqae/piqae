...
$profileHostPath = Join-Path $PSScriptRoot "piqae-profile-host-windows.exe"
foreach ($process in Get-Process -Name "piqae-profile-host-windows" -ErrorAction SilentlyContinue) {
    try {
...
$shellPath = Join-Path $PSScriptRoot "piqae-shell-windows.exe"
foreach ($process in Get-Process -Name "piqae-shell-windows" -ErrorAction SilentlyContinue) {
    try {
...
& (Join-Path $PSScriptRoot "Start-Piqae.ps1")
